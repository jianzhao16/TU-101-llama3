...
# Streamlit UI
st.markdown("# User Input - LLAMA 3 Model")
...
# Initialize session state for tracking the subprocess
...
if submit_button:
    ...
    if not st.session_state.is_running:
        ...
        st.info("Processing your request, please wait...  about 20 seconds.")

        # Run the subprocess
        #result_llama = run_subprocess()
        result_llama = subprocess.run(['./cmd-txt.sh'], capture_output=True, text=True)

        # Output from subprocess
        #st.text("STDOUT: " + result_llama.stdout)
        #st.text("STDERR: " + result_llama.stderr)

        # Reset the state after completion
        st.session_state.is_running = False
    else:
        st.warning("Already processing a request, please wait...  about 20 seconds.")

    if result_llama.stderr:

        # step 3. read and show result
            ...
            if raw_service_type and zipcode:
                ...
                if classified_service_type != "Other":
                    ...
                    if not location_info.empty:
                        ...
                        st.header(f"{classified_service_type} Services near {zipcode} --- LLAMA 3 Model")
                        #folium_static(map)
